...
def get_updated_cases(request):
    """
    This view is used by CourtListener to determine what to download from IA
    """
    api_keys = (u'',)
    ...
    docs = (docs.values(*columns)
            .order_by('court', 'casenum', 'docnum', 'subdocnum'))
    w = csv.DictWriter(response, fieldnames=columns)
    for doc in docs:
        w.writerow(doc)
    return response
